...
async def internal_webhook(request: Request):
    ...
    6. КОНКРЕТНОЕ ПОРУЧЕНИЕ (важно): В поле 'proposed_action' пиши конкретный приказ. Например: вместо "Сломан компьютер" пиши "Айти-специалист должен починить компьютер в 201 кабинете". Если "плохо ребенку" -> "Школьный врач должен осмотреть ученика и вызвать родителей". Оформляй в директивном стиле.    
