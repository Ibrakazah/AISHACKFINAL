...
async def internal_webhook(request: Request):
    ...
    3. Выдели СУТЬ в одно предложение (summary). Если это инцидент, укажи локацию.
    ...
    6. КОНКРЕТНОЕ ПОРУЧЕНИЕ (важно): В поле 'proposed_action' пиши конкретный приказ в директивном стиле. 
       - БОЛЕЗНЬ УЧЕНИКА: "Школьный врач должен осмотреть ученика и вызвать родителей".
       - БОЛЕЗНЬ УЧИТЕЛЯ/СОТРУДНИКА: "Завуч должен найти замену для учителя [Имя] и скорректировать расписание".
       - ТЕХНИЧЕСКАЯ ПОЛОМКА: См. список техперсонала (напр. "Слесарь Бекмуратов Серик должен починить кран в 101 кабинете").
       - ПИТАНИЕ/ОТСУТСТВИЕ: "Завпроизводством должен скорректировать количество порций согласно отчету".
